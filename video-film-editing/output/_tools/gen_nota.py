...
def set_cell_wa_list(cell, items, current_idx):
    """items: list of (code, title). Writes one line per WA; only current_idx is bold."""
    from docx.shared import Pt
    for para in list(cell.paragraphs)[1:]:
        para._element.getparent().remove(para._element)
    first = cell.paragraphs[0]
    for r in list(first.runs): r._element.getparent().remove(r._element)
    for i,(code,title) in enumerate(items):
        para = first if i==0 else cell.add_paragraph()
        para.paragraph_format.space_after = Pt(2)
        run = para.add_run(f"{code} / {title}")
        run.bold = (i==current_idx)
        run.font.size = Pt(10)

def fill_header_table(doc, prog_code_name, tahap, cu_code_title, wa_statements, kode_no, page_no, page_total, wa_items=None, wa_idx=0):
    t = doc.tables[0]
    ...
    if wa_items:
        set_cell_wa_list(t.rows[4].cells[1], wa_items, wa_idx)
    else:
    ...
    for ci in range(2, len(t.rows[1].cells)):
        try:
            ...
            if wa_items:
                if t.rows[4].cells[ci]._tc is not t.rows[4].cells[1]._tc:
                    set_cell_wa_list(t.rows[4].cells[ci], wa_items, wa_idx)
            elif t.rows[4].cells[ci].text != wa_statements:
                set_cell_text(t.rows[4].cells[ci], wa_statements)
        except IndexError:
            pass

def build_docx(md_path, out_path, cu_code, k, n, wa_items=None):
    ...
    fill_header_table(doc, prog, tahap, cu, wa, kode_no, page_no="1", page_total="[TBD: jumlah muka surat]", wa_items=wa_items, wa_idx=k-1)
...
def main():
    ...
    # WA list per CU from each md header: "IT-072-3:2012-C01-W02 / Analyse ..."
    wa_lists = {}
    for cu, base, f in order:
        h,_ = parse_md(f)
        stmt = hdr_lookup(h, "NO DAN PENYATAAN AKTIVITI") or hdr_lookup(h, "NO DAN NAMA WA") or ""
        m = re.match(r"\s*(IT-072-3:2012-C\d\d-W\d\d)\s*/\s*(.+)$", stmt)
        code = m.group(1) if m else base
        title = m.group(2).strip() if m else stmt
        wa_lists.setdefault(cu, []).append((code.split("-")[-1], title))
    seq = 0
    results = []
    for cu, base, f in order:
        seq += 1
        ...
        hdr = build_docx(f, out_path, cu, k, n, wa_items=wa_lists[cu])
