...
Every WIM begins with this block, delimited by HTML comment markers for idempotency. The logo + address are laid out side-by-side (logo left, address right) via an HTML 2-column table — matching the PDF reference exactly:
